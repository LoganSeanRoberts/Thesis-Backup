...
\section{The motivation fort Lattice QCD}\label{sec:motivation_for_LQCD}
...
From \cite{PhysRevD.67.054505}, the doubling in the temporal lattice direction gives rise to opposite parity states manifesting as oscillating terms $-(-1)^t$ in the correlation function. 

Appendix D in \cite{HISQ:2006rc} discusses how, in a flavor singlet case, there is no oscillating term in the correlator at zero meson momentum.  The quark and antiquark are created by the same field, and the only means by which they would have an interaction (which would have an amplitude), would be from annihilation and the emission of a highly off shell gluon.  Such gluons are removed by the contact terms discussed in that paper.
...
Citations \cite{Bedaque_2004, DEDIVITIIS2004408, Guadagnoli_2006}

\section{Correlation functions on the lattice}\label{sec:correlation_functions_lattice}


\begin{itemize}
    \item MILC code takes spin taste combination and implements as lattice site dependent phase to quark propogators
    \item Operator/Current $J(x_t)$ annihilates goldstone meson $\bar{h}l$ at $x_t$
    \item Current $J^\dagger(x_0)$ creates goldstone meson $\bar{h}l$ at $x_0$
    \item Two point function is then $\braket{J(x_t)J^\dagger(x_0)}$
    \item Wick contraction of this is equivalent to propagating $\bar{h}$ from $x_0$ to $x_t$, and propagating $l$ from $x_0$ to $x_t$.
    \item Out pops $\gamma_5 \otimes \xi_5$ requires no phase added to staggered quarks
    \item
\end{itemize}
...
The construction of two and three point correlators from HISQ quark bilinears in this work are as follows:
    ...
    \begin{split}
        ...
        S_{(s)} &= \bar{\psi}_h(I_4\otimes I_4)\psi_{l(s)} = \bar{\psi}_h\psi_{l(s)},\\
